...
function LoadModule {
    ...
    if (!(Get-Module -Name $name)) {
        $retVal = Get-Module -ListAvailable | Where-Object -FilterScript { $_.Name -eq $name }
        ...
        if ($retVal) {
            try {
                ...
                Write-Information -MessageData "The request to lload module $($name) failed with the following error:"
            ...
            if (Find-Module -Name $name) {
                ...
                Write-Information -MessageData "Module $($name) not imported, not available and not in online gallery, exiting."
                EXIT 1
            }
        }
    }
...
if ($SubscriptionID -like "*.csv") {
    $subscriptions = Import-Csv $SubscriptionID
}
elseif ($SubscriptionID -ne "") {
    $subscriptions = [PSCustomObject]@{SubscriptionId = $SubscriptionID } | Get-AzSubscription -TenantId $tenantID
}
else {
...
Write-Information -MessageData ([Environment]::NewLine + "-- Scanning subscriptions --")
...
foreach ($sub in $subscriptions) {
    ...
    try {
        ...
        Write-Information -MessageData "Invalid subscription: $($sub.Id)"
        { continue }
    }
    ...
    foreach ($r in $resources) {
        ...
        if ($LicenseType) {
            if (($LicenseType -eq "LicenseOnly") -and !$LO_Allowed) {
                Write-Information -MessageData "ESU must be disabled before license type can be set to $($LicenseType)"
            }
            else {
        ...
        if ($EnableESU) {
            if (($settings["LicenseType"] | Select-String "Paid", "PAYG") -or ($EnableESU -eq "No")) {
                ...
                Write-Information -MessageData "The configured license type does not support ESUs"
            }
        }

        If ($WriteSettings) {
            Write-Information -MessageData "Resource group: [$($r.resourceGroup)] Connected machine: [$($r.MachineName)] : License type: [$($settings["LicenseType"])] : Enable ESU: [$($settings["enableExtendedSecurityUpdates"])]"
            try {
                Set-AzConnectedMachineExtension @setId -Settings $settings -NoWait | Out-Null
            }
            catch {
                Write-Information -MessageData "The request to modify the extenion object failed with the following error:"
                Write-Information -MessageData $_.Exception.Message
                { continue }
            }
        }
